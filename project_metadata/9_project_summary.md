...
1. `wiz-v2_cicd_scan_policy` **`cst-container-vuln-<cluster>`** — Vulnerability scan policy, a separate instance copying the golden default's baseline params.
...
every cluster so each `cst-container-vuln-<cluster>` copies the new baseline. **No version
...
against each target cluster's `cst-container-vuln-<cluster>`, and `wiz tag`s on pass so
