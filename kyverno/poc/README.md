...
extensions** (`wiz/.../scripts/gen_signing_certs.sh`). notation-core-go scores a cert with no
...
export CA_CRT="../wiz/container-vulnerability-exemption/trust/ca.crt"
...
| 5.10 | Digest mutation | `mutateDigest: true`, submit a **tag** reference | Pod's `spec.containers[].image` is rewritten to `...@sha256:...`. Check that this composes with the base-comparison-by-digest model in `wiz/` rather than fighting it |
